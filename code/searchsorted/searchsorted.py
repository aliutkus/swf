...
try:
    from .searchsorted_wrapper import searchsorted_cpu_wrapper
except ImportError:
...
try:
    from .searchsorted_wrapper import searchsorted_cuda_wrapper
except ImportError:
...
def searchsorted(a, v, out=None):
    ...
    if a.is_cuda and not SEARCHSORTED_GPU_AVAILABLE:
        raise Exception('searchsorted on CUDA device is asked, but it seems '
                        'that it is not available. Please run `make`.')
