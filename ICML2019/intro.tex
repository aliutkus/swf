...
To the best of our knowledge, the proposed algorithm is the first nonparametric IGM algorithm that has explicit theoretical guarantees. In addition to its nice theoretical properties, the proposed algorithm has also significant practical importance: it has low computational requirements and can be easily run on an everyday laptop CPU.%\footnote{The code is publicly available on \url{https://github.com/aliutkus/SWF}.}.
 % On the other hand, it might have interesting implications in other domains, such as differential-privacy \cite{dwork2014algorithmic}, since it only requires random projections of the data, rather than the data itself.
