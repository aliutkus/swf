...
For a compact domain $\Omega \in \R^d$, the $\W$-distance forms a distance over $\PS(\Omega)$, such that convergence in $\W$ is equivalent to the weak convergence of probability measures (cf.\ \cite{santambrogio2010introduction}[Theorem 2.1]). The metric space $(\PS(\Omega),\W) $ is often called the \emph{Wasserstein space} and is denoted as $\WS(\Omega)$ \cite{santambrogio2017euclidean}. 

In this study, we are interested in functional optimization problems in $\WS$, such as $\min_{\mu\in \WS(\Omega)} \F(\mu)$, where $\F$ is the functional that we would like to minimize. Similar to Euclidean spaces, one way to formulate this optimization problem is to construct a gradient flow of the form $\partial_t \mu_t = - \nabla_{\W} \F(\mu_t)$, where $\nabla_{\W}$ denotes a notion of gradient in Wasserstein spaces. If such a flow can be constructed, then one can attempt to utilize it both for developing practical algorithms and theoretical analysis.   

The gradient flows in $\WS$ have strong connections with partial differential equations (PDE) of the form of a continuity equation. Assuming that the measure $\mu_t$ admits a density, denoted as $\rho_t$, such a continuity equation would have the following form:
\begin{align}
\partial_t \rho_t + \nabla \cdot (v \rho_t) = 0,  \label{eqn:pde}
\end{align}  
where $v$ denotes a vector field and $\nabla \cdot (v \rho_t)$ denotes the divergence of the vector field $v\rho_t$. Then, for a given gradient flow in $\WS$, we are interested in the evolution of the densities $\rho_t$, i.e.\ the PDEs which they solve. 
%
In order to prove that the path $(\rho_t)_t$ solves a PDE for a given gradient flow, one first needs to show that there exits a path $(\rho_t)_t$ that appears as the limit of the solution of a \emph{time-discretized} problem \cite{jordan1998variational,santambrogio2017euclidean}. The form of the vector field $v$ in \eqref{eqn:pde} can only be determined by further analysis. We provide more information about gradient flows in $\WS$ in the supplementary document.
