...
\begin{align}
\hat{v}_k(x) \triangleq - \frac1{N_\theta} \sum_{n=1}^{N_\theta} \psi_{k, \theta_{nk}}'(\langle\theta_{nk},x\rangle ) \theta_{nk}, \label{eqn:approxdrift}
\end{align}
...
\begin{align}
   \psi_{k, \theta}'(z) = \Bigl[ (F^{-1}_{\theta^*_\#\bar{\mu}_{kh}^{(N)}} \circ F_{\theta^*_\#\nu}) (z) - z \Bigr].
 \end{align}
