...
\section{Practical Simulation Algorithm for the SDE}
...
We now focus on the first issue. In order to highlight the fact that the drift in \eqref{eqn:sde} depends on $\mu_t$, within this section we will denote the drift as $v_t(x) \equiv v(x, \mu_t)$. With this notation, it can be more clearly observed that the SDE \eqref{eqn:sde} is similar to McKean-Vlasov SDEs \cite{veretennikov2006ergodic,mishura2016existence} whose drifts also depend on the distribution of $X_t$. By exploiting this connection, we borrow tools from the relevant SDE literature \cite{malrieu03,cgm-08} for developing and approximate simulation method for \eqref{eqn:sde}. 

Our approach is based on defining a \emph{particle system}, that is given as follows:
\begin{align}
d X_t^i = - v(X_t^i, \tilde{\mu}_t^{(N)}) dt + \sqrt{2 \lambda } d W_t^i \> , \quad i = 1,\dots, N,
\end{align}
where $i$ denotes the particle index, $N$ denotes the total number of particles, and $\tilde{\mu}_t^{(N)}$ denotes the law of the particles $\{X_t^j\}_{j=1}^N$. 
