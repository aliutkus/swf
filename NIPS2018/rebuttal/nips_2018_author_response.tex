...
In this sense, our main contributions are mostly theoretical and conceptual, which are then validated by our experiments, whose goal is to provide a `proof-of-concept', rather than to try to achieve state-of-the-art results. This is the reason why we mostly focused on validating our algorithm, rather than comparing it with the existing approaches. On the other hand, unfortunately it is already very difficult the assess the validity of IGM algorithms. \emph{The most common way is to visualize the generated samples, as is done in our paper.} To enable comparison with other algorithms, we will include their visual results in the supp.\ \vspace{2pt} \\
%
