...
\end{align}
where $k \in \mathbb{N}_+$ denotes the iteration number, $\{Z_k\}_{k}$ denotes a series of standard Gaussian random variables, $h$ denotes the step-size, and $\hat{v}_k$ is a computable unbiased estimator of $v_k$.
...
We are interested in computing the distance $\| \muh_{kh} - \nu_\lambda \|_{\TV}$, where $\muh_{kh}$ denotes the law of $\hat{X}_k$ with step size $h$. In order to upper-bound this distance, we follow the approach presented in \cite{dalalyan2017theoretical} and \cite{raginsky17a}, where we decompose the into two terms: $\| \muh_{kh} - \nu_\lambda \|_{\TV} \leq \| \muh_{kh} - \mu_t \|_{\TV} + \| \mu_{t} - \nu_\lambda \|_{\TV}$, where $\mu_t$ denotes the law of $X_t$ such that $(X_t)_t$ is the solution of the continuous-time SDE \eqref{eqn:sde}. 
...
\begin{align}
\| \muh_{Kh} - \mu_{T} \|_{\TV}^2 \leq \frac{L^2 K}{4\lambda} \Bigl( \frac{C_1 h^3}{3} + \frac{6 \lambda d h^2}{2} \Bigr) + \frac{C_2 K h}{8\lambda},
\end{align}
...
\end{align}
By Lemma 3.2 of \cite{raginsky17a}, we have $\E[ \|Y_{kh}\|^2] \leq C_0 \triangleq 2 \max(1, \frac1{m})(b+2B^2 + d \lambda)$. Using this result in the above equation yields:
\begin{align}
...
=& \frac{L^2 K}{\lambda} \Bigl( \frac{C_1 h^3}{3} + \frac{6 \lambda d h^2}{2} \Bigr) + \frac{C_2 K h}{2\lambda},
\end{align}
where $C_1 = 12(L^2 C_0 + B^2)+1$ and $C_2 = 2 \delta(L^2 C_0 + B^2)$.
...
=& \frac{L^2 K}{4\lambda} \Bigl( \frac{C_1 h^3}{3} + \frac{6 \lambda d h^2}{2} \Bigr) + \frac{C_2 K h}{8\lambda}.
\end{align}
